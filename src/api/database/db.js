...
const insertExchangeKeys = async (params) => {
    const exchange = params[0]
    const { rows } = await db.query('SELECT * FROM exchange_keys WHERE exchange = $1', [exchange])
...
const selectKeysByExchange = async (params) => {
    const { rows } = await db.query('SELECT * FROM exchange_keys WHERE exchange = $1', params)
...
const insertBotKeys = async (params) => {
    const { rows } = await db.query('SELECT * FROM bot_keys WHERE bot_id = $1 AND bot_key = $2 AND exchange = $3', params)
...
const selectKeysByBotId = async (params) => {
    const { rows } = await db.query('SELECT * FROM bot_keys WHERE bot_id = $1', params)
...
const selectAllKeys = async () => {
    const { rows } = await db.query('SELECT * FROM bot_keys')
...
const selectLatestOrder = async (params) => {
    const defaultStart = "2007-01-01T00:00:00.000Z"

    const { rows } = await db.query('SELECT _timestamp FROM orders WHERE _timestamp = ( SELECT max ("_timestamp") FROM  orders ) AND bot_id = $1', params)
    if (!rows.length) {
        return defaultStart
    }
...
const selectOrdersByStatus = async (params) => {
    const { rows } = await db.query('SELECT * FROM orders WHERE order_status = $1 AND position_ref IS NULL ORDER BY id ASC', params)
...
const selectPaperOrders = async () => {
    const { rows } = await db.query('SELECT * FROM paper_orders')
    return rows
}
...
const selectPaperOrdersByStatus = async (params) => {
    const { rows } = await db.query('SELECT * FROM paper_orders WHERE order_status = $1 AND position_ref IS NULL ORDER BY id ASC', (params))
...
const selectWebsocketByExchange = async (params) => {
    const { rows } = await db.query('SELECT * FROM websockets WHERE exchange = $1', params)
...
const insertWebsocket = async (params) => {
    const { rows } = await db.query('SELECT * FROM websockets WHERE exchange = $1 AND asset = $2 AND time_frame = $3', params)
...
const selectAllPriceHistory = async (params) => {
    const { rows } = await db.query('SELECT * FROM price_history WHERE time_frame = $1 AND pair = $2 AND exchange = $3 ORDER BY _timestamp ASC', params)
...
const selectLatestPriceHistory = async (params) => {
    const { rows } = await db.query('SELECT _timestamp FROM price_history WHERE time_frame = $1 AND pair = $2 AND exchange = $3 ORDER BY _timestamp DESC', params)
...
const insertPriceHistory = async (params) => {
    const { rows } = await db.query('INSERT INTO price_history(pair, time_frame, exchange, _timestamp, _open, _close, _high, _low, _volume) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)', params)
...
const selectMargin = async (params) => {
    const { rows } = await db.query('SELECT * FROM MARGIN WHERE AMOUNT = $1 AND _timestamp = $2', params)
...
const insertBotStrategy = async (params) => {
    const checkVar = params[0]
    const { rows } = await db.query('SELECT * FROM bots WHERE bot_id = $1', [checkVar])
...
const selectBotByBotId = async (params) => {
    const { rows } = await db.query('SELECT * FROM bots WHERE bot_id = $1 ', params)
    return rows
}
...
const updateBotStrategyStatus = async (params) => {
    const { rows } = await db.query('UPDATE bots SET _status = $1 WHERE bot_id = $2', params)
    return rows
}
...
const selectPaperPositions = async () => {
    const { rows } = await db.query('SELECT * FROM paper_position')
    return rows
}
 ...
 */
const insertTrade = async (params) => {
    for (let i = 0; i < params.length; i+=1) {
        await db.query('INSERT INTO backtest_trade(symbol, side, order_qty, price, order_type, time_in_force, _timestamp, leverage) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)', params[i])
    }
}
...
module.exports = { 
    insertExchangeKeys, selectKeysByExchange, insertBotKeys, selectAllKeys, selectKeysByBotId, insertOrder, selectLatestOrder, selectOrdersByStatus, selectOrders, 
    updateOrderStatus, updateOrderPositionId, insertPaperOrder, selectPaperOrders, selectPaperOrdersByStatus, updatePaperOrderStatus, updatePaperOrderPositionId, selectWebsocketByExchange, 
    insertWebsocket, selectAllPriceHistory, selectLatestPriceHistory, insertPriceHistory, selectBotByBotId, insertBotStrategy, updateBotStrategyStatus, updateBotMargin, insertPosition, 
    selectPositions, insertPaperPosition, selectPaperPositions, insertMargin, selectMargin, selectPerformance, insertPerformance, cleanTrade, insertTrade, selectAllTrades, TruncateTables 
}