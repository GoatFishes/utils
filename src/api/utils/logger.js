...
*/
const logEvent = async (logLevel, responseCode, message, arg) => {
    try {
        let args
        (arg === undefined) ? args = '' : args = arg
        const initial = responseCode === RESPONSE_CODES.LOG_MESSAGE_ONLY ? '' : 'RESPONSE CODE : ' + responseCode + ' - '

        switch (logLevel) {
            case LOG_LEVELS.info:  await global.logger.info (initial + 'EVENT MESSAGE : ' + message + args); break;
            case LOG_LEVELS.warn:  await global.logger.info (initial + 'EVENT MESSAGE : ' + message + args); break;
            case LOG_LEVELS.error:  await global.logger.info (initial + 'EVENT MESSAGE : ' + message + args); break;
