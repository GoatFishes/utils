...
const makeConsumerInstance = (topic, offsets) => new Consumer(
    new kafka.KafkaClient({ kafkaHost: 'kafka:9092' }),
    [{ topic, offsets, partition: 0 }],
    { autoCommit: true },
)
...
const consume = consumerInstance => new Promise((resolve, reject) => {
    ...
    consumerInstance.on('message', message => {
        ...
        if (message.offset === (message.highWaterOffset - 1)) {
            consumerInstance.close(true, function (err, msg) { })
            resolve(messages)
...
const groupConsumer = async (groupId, topic) => {
    ...
    const consumerGroup = liveTradeConsumer
...
const pauseConsumer = async (groupId) => {
    ...
    await consumerGroup.close(true, function (err, msg) { });
}
